...
const ApplicationForm = ({ open, onOpenChange }: ApplicationFormProps) => {
  ...
  const requiredSchemas = useMemo(() => {
    ...
    return {
      ...
      step1: z.object({
        ...
        timeline: z.string().trim().min(1, { message: "Please select a timeline" }),
      }),
      step2: z.object({
  ...
  // Auto-redirect to consultation modal 3 seconds after submission
  useEffect(() => {
    if (!submitted) return;
          ...
          <DialogContent className="max-w-xl p-0 overflow-hidden">
            <div className="bg-primary p-8 md:p-10 text-primary-foreground">
              <div className="text-center">
                ...
                <h2 className="font-serif text-2xl md:text-3xl font-bold mb-4">Thank You</h2>

                <p className="text-primary-foreground/90 mb-4 max-w-md mx-auto">
                  Your form is submitted. Our team will reach out to you.
                </p>

                <p className="text-primary-foreground/60 text-sm">
                  Redirecting to book consultation...
                </p>
              </div>
            </div>
          </DialogContent>
        </Dialog>
        <ScheduleCallModal open={showScheduleModal} onClose={handleScheduleModalClose} />
      </>
    );
  }
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden p-0">
          ...
          <div className="p-6">
            ...
            <div ref={scrollContainerRef} className="overflow-y-auto max-h-[45vh] pr-2">
                      ...
                      <div>
                        ...
                        >
                          {[
                            ...
                            <div
                              key={option.value}
                              className="flex items-center space-x-3 border border-border rounded-sm p-4 hover:bg-muted transition-colors cursor-pointer"
                              onClick={() => handleInputChange("timeline", option.value)}
                            >
                              <RadioGroupItem value={option.value} id={`timeline-${option.value}`} />
                              <Label htmlFor={`timeline-${option.value}`} className="flex-1 cursor-pointer text-sm">
                                {option.label}
                              </Label>
                            </div>
                          ))}
